...
const EditPet = () => {
  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        ...
        setTimeout(() => navigate('/pet-management'), 2000);
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await API.put(`/pets/${id}`, formDataToSend, {
        headers: {
      ...
      const errorMessage = err.response?.data?.msg || err.message || "Failed to update pet";
      setPopup({ open: true, message: errorMessage, severity: "error" });
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Update the pet information</p>
        </div>
